...
3. **Agent mutation** — un agent distinct, PAS celui qui a écrit le code : il lance Stryker et **instruit chaque survivant** (équivalent toléré, ou vrai trou de test avec le scénario non couvert). L'étape se **déclenche mécaniquement**, jamais sur un jugement : elle a lieu si `git diff --name-only` croise le périmètre `mutate` de `stryker.conf.mjs`, et se saute sinon. Un lot qui ne touche que des `.tsx` ne perd rien en la sautant — ils ne sont mutés par rien. « Ce lot ne touche pas la logique » est exactement l'affirmation de complétude que la section précédente interdit de dire de mémoire.
...
- **Le score global n'est PAS reproductible : le run isolé fait foi, machine au repos.** Un même mutant est compté « tué par timeout » ou « survivant » selon la charge machine. Pour **tout fichier modifié dans un cycle**, rejouer `npm run test:mutation:isolated -- '<fichier>'` et **rapporter ce chiffre-là** ; le global ne vaut que comme signal de fumée.
- **La concurrence est déclarée à 4** (`stryker.conf.mjs`), et non plus laissée au défaut de Stryker — `n-1` workers, où `n` dépend de la machine ([ADR 0042](docs/decisions/0042-la-concurrence-de-stryker-est-declaree.md)). Sans ce plafond, Stryker se saturait lui-même et les timeouts noyaient de vrais survivants. Les scores rendus **avant le 2026-09-10** l'ont été à concurrence par défaut sur un poste à 22 cœurs, donc 21 workers : ils se lisent dans ce contexte.
...
- **Tout survivant qui entraîne un changement — test ajouté, code supprimé, directive posée — part en issue**, même corrigé dans le cycle, avec sa provenance `agent mutation` et le verdict rendu (équivalent, ou vrai trou avec le scénario non couvert). Sans cette trace, la mutation est le seul garde-fou dont on ne peut pas mesurer le rendement : ce qu'elle attrape se referme dans le cycle et ne laisse rien derrière. **Motif mesuré** : sur 226 commits, l'inventaire n'a trouvé **aucun** survivant ayant révélé un défaut de production — et n'a pas pu dire si c'est parce qu'il n'y en a pas eu, ou parce qu'aucun n'a été écrit quelque part.
...
6. Si un finding a été appliqué, **le flux repart à l'étape 1** — nouveau cycle, nouvelle vérif Chrome, re-revue sur le **delta**. **Pas de nouvelle mutation à ce tour** : elle attend le run unique d'avant commit.
...
**L'agent principal ne rejoue pas systématiquement `lint`/`test`/`build` après chaque rapport d'agent** : un seul passage complet avant commit suffit. **La mutation suit la même règle** — elle tourne **une fois, avant commit**, sur l'ensemble des fichiers modifiés par la branche, pas à chaque tour de rebouclage. Un correctif de revue qui affaiblirait une assertion est attrapé par ce run final.

C'est la ligne qui coûtait le plus cher : une revue trouve presque toujours quelque chose, et chaque finding retenu repayait un run de mutation complet.
...
- L'issue porte sa **provenance** — qui l'a trouvée, en première ligne —, le **scénario concret**, les références `fichier:ligne`, et le classement (introduit/pré-existant, bloquant/non bloquant) : elle doit être reprenable sans le contexte de la session qui l'a produite. Gabarit : `.github/ISSUE_TEMPLATE/finding.md`. Regrouper est permis et souvent préférable — plusieurs findings qui se referment dans la même passe font une seule issue.
- **Sans provenance, on ne peut pas savoir quel garde-fou attrape quoi**, donc pas savoir lequel supprimer. **Motif mesuré** : 58 issues sur 87 ne disaient pas qui les avait trouvées, et l'inventaire des garde-fous a dû se rabattre sur un rang au lieu d'un compte.
...
- [ ] **Si `git diff --name-only` croise le périmètre `mutate`** — sinon les deux cases suivantes sont **sans objet**, et le rapport le dit avec le diff à l'appui :
- [ ] `npm run test:mutation` OK (seuil `break: 80` tenu — gate bloquant, pas décoratif)
- [ ] **Run de mutation ISOLÉ** (`npm run test:mutation:isolated -- '<fichier>'`) sur chaque fichier modifié, **une fois, avant commit** — pas à chaque tour de rebouclage. C'est **ce** chiffre qui est rapporté, timeouts compris — `npx stryker run --mutate` seul fusionne le cache incrémental et rend un chiffre qui n'est pas celui du fichier
