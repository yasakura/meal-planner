...
**La règle vaut aussi pour les instruments de MESURE, et pas seulement pour les garde-fous.** Un garde-fou muet ressemble à la conformité ; un instrument déréglé, lui, affiche un chiffre **plausible**, ce qui est pire — on ne se méfie pas d'un nombre. Toute commande dont on rapporte le résultat doit donc avoir été confrontée à un cas dont on connaît la réponse d'avance, et **re-confrontée à chaque fois qu'on touche à sa configuration**. C'est exactement ce qui a manqué le 2026-08-17 : activer `incremental: true` a silencieusement cassé la commande de run isolé, et le chiffre est resté crédible pendant un cycle entier. _(Deux occurrences le même jour, 2026-08-18 : la règle `boundaries` inerte depuis l'origine du projet, et le run de mutation isolé fusionné avec le cache. Les deux ont été trouvées parce qu'un agent a refusé de prendre son propre outil au mot, aucune par la lecture du code.)_
...
- **Le score global n'est PAS reproductible : le run isolé fait foi.** Avec `timeoutMS: 10000` et la concurrence par défaut, un même mutant est compté « tué par timeout » quand la machine est chargée et « survivant » quand elle respire. Sur un même commit, `convives-slice.ts` est sorti à **100 % en run complet et 85 % en run ciblé** (2026-08-12). Pour **tout fichier modifié dans un cycle**, rejouer `npm run test:mutation:isolated -- '<fichier>'` et **rapporter ce chiffre-là** ; le global ne vaut que comme signal de fumée. Conséquence : un survivant réel peut se cacher derrière un timeout, et le score global ne vaut rien comme mesure de progression.
- **`--mutate` seul ne suffit PAS à isoler, et c'est un piège silencieux.** Avec `incremental: true`, `npx stryker run --mutate '<fichier>'` relit `reports/` et **fusionne les résultats en cache de tous les autres fichiers** dans le tableau et dans le score. Il affiche donc un chiffre plausible, mais qui n'est pas celui du fichier demandé. Mesuré le 2026-08-18 sur `recipe-slice.ts` : **99,36 % avec 49 timeouts et 5 survivants** par la commande fusionnée, contre **100 % avec 19 mutants, 0 survivant et 0 timeout** en run réellement isolé — aucun des survivants affichés ne concernait le fichier. Le script `test:mutation:isolated` détourne le cache vers un fichier jetable **et le supprime avant chaque run** : sans cette suppression le fichier jetable redevient lui-même un cache, et le défaut se reforme d'un cycle à l'autre. `--no-incremental` n'existe pas, et `--force` reconstruirait le cache réel en le limitant au fichier ciblé — il détruirait l'incrémental des autres.
- **Le run isolé règle la fusion, pas les timeouts.** Un fichier peut sortir à 100 % en run isolé **avec des timeouts** (mesuré : `convive.ts`, 15 tués + 3 timeouts). Stryker compte un timeout comme un mutant tué : l'optimisme structurel demeure, et un survivant réel peut toujours s'y cacher. Un score isolé assorti de timeouts se rapporte **avec** son nombre de timeouts, jamais nu.
